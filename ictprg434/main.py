...
def start():
		...
		if bool(re.search(r"\bLinux\b", getSystemOS())) == True:
			...
			try:
				...
				wmic_output = subprocess.check_output(
					"wmic cpu get name, numberofcores, maxclockspeed", shell=True
				).decode()
		...
		for interface_name, addresses in interfaces.items():
			for address in addresses:
				if address.family == socket.AF_INET and address.address == target_ip:
					...
					# return "Missing information: Nic speed" # Throw Error if Nic speed or interface isn't found
					return interfaces
		# print(interfaces)
	...
	# 	"MAC-address": getSystemMACAddress(
	# 		getSystemNetworkInterfaceName(getSystemIPv4Address())
	# 	),
	...
	# 	# Only Write to the Header if it doesn't exprint
	...
	# print(getSystemNetworkSpeed(getSystemNetworkInterfaceName(getSystemIPv4Address())))
	return getSystemNetworkInterfaceName(getSystemIPv4Address())
